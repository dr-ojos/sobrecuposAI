// Configuración de áreas de interés por especialidad médica
export const AREAS_INTERES = {
  "Oftalmología": [
    "Cirugía refractiva Láser",
    "Córnea", 
    "Glaucoma",
    "Retina",
    "Cirugía Plástica",
    "Neurooftalmología", 
    "Oftalmopediatría",
    "Cataratas",
    "Lentes Intraoculares",
    "Uveítis",
    "Atopia Ocular",
    "Ojos Secos",
    "Estética"
  ],

  "Otorrinolaringología": [
    "Oídos",
    "Rinoplastía",
    "Otorrinopediatría", 
    "Laringe",
    "Alergias",
    "Oncología",
    "Audiología",
    "Vértigo y Mareos",
    "Cirugía de Cabeza y Cuello",
    "Ronquidos y Apnea"
  ],

  "Dermatología": [
    "Acné",
    "Neoplasias",
    "Estética",
    "Rosácea",
    "Alergias", 
    "Nevus",
    "Oncología",
    "Dermopediatría",
    "Psoriasis",
    "Dermatitis",
    "Láser Dermatológico",
    "Tricología (Cabello)"
  ],

  "Medicina Familiar": [
    "Medicina Preventiva",
    "Control de Crónicos",
    "Diabetes",
    "Hipertensión",
    "Medicina Geriátrica",
    "Medicina del Adolescente",
    "Medicina Laboral",
    "Vacunación",
    "Medicina Deportiva",
    "Nutrición"
  ],

  "Medicina Familiar Niños": [
    "Control Niño Sano",
    "Vacunación Infantil",
    "Crecimiento y Desarrollo",
    "Enfermedades Respiratorias",
    "Gastroenterología Infantil",
    "Alergias Pediátricas",
    "Dermatitis y Eczemas",
    "Infecciones Comunes",
    "Nutrición Infantil",
    "Medicina del Adolescente"
  ],

  "Medicina Familiar Adultos": [
    "Check-up Preventivo",
    "Enfermedades Crónicas",
    "Diabetes Tipo 2",
    "Hipertensión Arterial",
    "Dislipidemia",
    "Medicina Geriátrica",
    "Medicina Laboral",
    "Medicina Deportiva",
    "Salud Mental",
    "Medicina Preventiva"
  ],

  "Pediatría": [
    "Neonatología",
    "Crecimiento y Desarrollo",
    "Vacunación Infantil",
    "Alergias Pediátricas",
    "Gastroenterología Pediátrica",
    "Neurología Pediátrica",
    "Endocrinología Pediátrica",
    "Cardiología Pediátrica",
    "Urgencias Pediátricas",
    "Adolescencia"
  ],

  "Cardiología": [
    "Ecocardiografía",
    "Electrocardiografía",
    "Holter",
    "Test de Esfuerzo",
    "Hipertensión Arterial",
    "Arritmias",
    "Insuficiencia Cardíaca",
    "Cardiopatía Isquémica",
    "Prevención Cardiovascular",
    "Cardiología Deportiva"
  ],

  "Ginecología": [
    "Embarazo y Control Prenatal",
    "Planificación Familiar",
    "Menopausia",
    "Endocrinología Ginecológica",
    "Oncología Ginecológica",
    "Cirugía Ginecológica",
    "Fertilidad",
    "Ecografía Ginecológica",
    "Enfermedades de Transmisión Sexual",
    "Adolescencia Ginecológica"
  ],

  "Traumatología": [
    "Columna Vertebral",
    "Rodilla",
    "Hombro",
    "Cadera",
    "Mano y Muñeca",
    "Pie y Tobillo",
    "Medicina Deportiva",
    "Artroscopía",
    "Fracturas",
    "Ortopedia Pediátrica"
  ],

  "Neurología": [
    "Cefaleas y Migrañas",
    "Epilepsia",
    "Parkinson",
    "Alzheimer y Demencias",
    "Esclerosis Múltiple",
    "Electroencefalografía",
    "Electromiografía",
    "ACV (Accidente Cerebrovascular)",
    "Trastornos del Sueño",
    "Neuropatías"
  ],

  "Psiquiatría": [
    "Depresión",
    "Ansiedad",
    "Trastorno Bipolar",
    "Esquizofrenia",
    "Trastornos de la Personalidad",
    "Adicciones",
    "Psiquiatría Infantil",
    "Psiquiatría Geriátrica",
    "Trastornos Alimentarios",
    "TDAH"
  ],

  "Urología": [
    "Próstata",
    "Cálculos Renales",
    "Incontinencia Urinaria",
    "Disfunción Eréctil",
    "Infecciones Urinarias",
    "Oncología Urológica",
    "Andrología",
    "Pediatría Urológica",
    "Laparoscopía Urológica",
    "Trasplante Renal"
  ],

  "Endocrinología": [
    "Diabetes",
    "Tiroides",
    "Obesidad",
    "Osteoporosis",
    "Hormonas",
    "Síndrome Metabólico",
    "Endocrinología Reproductiva",
    "Suprarrenales",
    "Hipófisis",
    "Endocrinología Pediátrica"
  ],

  "Gastroenterología": [
    "Endoscopía Digestiva",
    "Colonoscopía",
    "Enfermedad Inflamatoria Intestinal",
    "Síndrome Intestino Irritable",
    "Hígado Graso",
    "Hepatitis",
    "Reflujo Gastroesofágico",
    "Úlcera Péptica",
    "Oncología Digestiva",
    "Nutrición Clínica"
  ],

  "Broncopulmonar": [
    "Asma",
    "EPOC",
    "Neumonía",
    "Apnea del Sueño",
    "Espirometría",
    "Fibrosis Pulmonar",
    "Oncología Pulmonar",
    "Tuberculosis",
    "Alergias Respiratorias",
    "Medicina del Sueño"
  ],

  "Reumatología": [
    "Artritis Reumatoide",
    "Lupus",
    "Fibromialgia",
    "Osteoartritis",
    "Gota",
    "Espondilitis Anquilosante",
    "Síndrome de Sjögren",
    "Vasculitis",
    "Osteoporosis",
    "Artritis Psoriásica"
  ],

  "Oncología": [
    "Oncología Médica",
    "Quimioterapia",
    "Inmunoterapia",
    "Cáncer de Mama",
    "Cáncer Colorrectal",
    "Cáncer de Pulmón",
    "Hematología",
    "Cuidados Paliativos",
    "Oncología Pediátrica",
    "Medicina Nuclear"
  ],

  "Hematología": [
    "Anemia",
    "Leucemia",
    "Linfomas",
    "Trastornos de Coagulación",
    "Mieloma Múltiple",
    "Trasplante de Médula Ósea",
    "Hemofilia",
    "Trombosis",
    "Hematología Pediátrica",
    "Banco de Sangre"
  ],

  "Nefrología": [
    "Insuficiencia Renal Crónica",
    "Hemodiálisis",
    "Diálisis Peritoneal",
    "Trasplante Renal",
    "Hipertensión Arterial",
    "Proteinuria",
    "Glomerulonefritis",
    "Cálculos Renales",
    "Electrolitos",
    "Nefrología Pediátrica"
  ],

  "Infectología": [
    "VIH/SIDA",
    "Hepatitis Virales",
    "Tuberculosis",
    "Infecciones Hospitalarias",
    "Medicina del Viajero",
    "Vacunación",
    "Parasitología",
    "Infecciones Pediátricas",
    "Control de Infecciones",
    "Enfermedades Tropicales"
  ],

  "Geriatría": [
    "Demencias",
    "Fragilidad",
    "Caídas",
    "Polifarmacia",
    "Medicina Preventiva",
    "Cuidados Paliativos",
    "Nutrición Geriátrica",
    "Rehabilitación Geriátrica",
    "Psicogeriatría",
    "Medicina Familiar Geriátrica"
  ],

  "Medicina Interna": [
    "Medicina Hospitalaria",
    "Enfermedades Autoinmunes",
    "Medicina Crítica",
    "Diabetes",
    "Hipertensión",
    "Medicina Preventiva",
    "Enfermedades Infecciosas",
    "Medicina Basada en Evidencia",
    "Ética Médica",
    "Medicina Paliativa"
  ],

  "Nutriólogo": [
    "Nutrición Clínica",
    "Diabetes y Alimentación",
    "Obesidad y Sobrepeso",
    "Nutrición Deportiva",
    "Nutrición Pediátrica",
    "Trastornos Alimentarios",
    "Nutrición Geriátrica",
    "Alimentación Vegana y Vegetariana",
    "Síndrome Metabólico",
    "Nutrición Oncológica",
    "Hipertensión y Dieta",
    "Intolerancias Alimentarias",
    "Nutrición Materno-Infantil",
    "Educación Nutricional"
  ]
};

// Función para obtener áreas por especialidad
export const getAreasByEspecialidad = (especialidad) => {
  return AREAS_INTERES[especialidad] || [];
};

// Función para obtener todas las especialidades disponibles
export const getEspecialidadesDisponibles = () => {
  return Object.keys(AREAS_INTERES);
};

// Función para buscar áreas que contengan un texto
export const searchAreas = (searchText) => {
  const results = [];
  Object.entries(AREAS_INTERES).forEach(([especialidad, areas]) => {
    const matchingAreas = areas.filter(area => 
      area.toLowerCase().includes(searchText.toLowerCase())
    );
    if (matchingAreas.length > 0) {
      results.push({
        especialidad,
        areas: matchingAreas
      });
    }
  });
  return results;
};

// Función para validar si un área pertenece a una especialidad
export const isValidArea = (especialidad, area) => {
  const areas = getAreasByEspecialidad(especialidad);
  return areas.includes(area);
};