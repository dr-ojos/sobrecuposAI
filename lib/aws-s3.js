...
export const uploadImageToS3 = async (
  ...
  try {
    ...
    const command = new PutObjectCommand({
      ...
      // Hacer la imagen pública para evitar problemas de desktop
      ACL: 'public-read',
    ...
    // Generar URL pública directa (sin firma)
    const publicUrl = `https://${BUCKET_NAME}.s3.${process.env.AWS_REGION}.amazonaws.com/${uniqueFileName}`;
    
    console.log('✅ Imagen subida exitosamente a S3 como pública:', publicUrl);
    
    return {
      success: true,
      url: publicUrl, // Usar URL pública en lugar de firmada
      key: uniqueFileName,
      bucket: BUCKET_NAME,
      etag: result.ETag
    };
