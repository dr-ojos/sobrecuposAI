...
import { Sobrecupo, SobrecupoFields } from '@/types/sobrecupo';
...
interface MedicoInfoPageProps {
  params: Promise<PageParams>;
}

// Calendar date info interface
...
interface DoctorsApiResponse extends Array<Doctor> {}

export default function MedicoInfoPage({ params }: MedicoInfoPageProps): React.JSX.Element {
  const router = useRouter();
  const [medico, setMedico] = useState<Doctor | null>(null);
  ...
  useEffect(() => {
    const extractSlug = async (): Promise<void> => {
  ...
  const fetchSobrecuposMedico = async (nombreMedico: string): Promise<void> => {
    try {
      ...
      if (data.success && data.records) {
        ...
        const sobrecuposMedico = data.records.filter((sobrecupo: Sobrecupo) => {
          const fechaSobrecupo = new Date(sobrecupo.fields?.Fecha);
          fechaSobrecupo.setHours(0, 0, 0, 0);
          
          return sobrecupo.fields?.Médico?.includes(nombreMedico) &&
        ...
        sobrecuposMedico.forEach((sobrecupo: Sobrecupo) => {
  ...
  const handleDateClick = (dateInfo: CalendarDateInfo | null): void => {
    if (dateInfo && dateInfo.isAvailable && medico?.fields?.Name) {
  ...
  useEffect(() => {
    const fetchMedicoInfo = async (): Promise<void> => {
      try {
        setLoading(true);
        ...
        const doctores: DoctorsApiResponse = await response.json();
        console.log('📋 Doctores obtenidos:', doctores.length);
        
        // Buscar médico por nombre exacto o similar
        const doctorEncontrado = doctores.find((doc: Doctor) => {
  ...
  const fields: DoctorFields = medico.fields || {} as DoctorFields;
          ...
          <div className="mobile-layout">
              ...
              <div className="calendar-container">
                ...
                <div className="calendar-body">
                  <div className="calendar-weekdays">
                    {['L', 'M', 'X', 'J', 'V', 'S', 'D'].map((day: string) => (
                      <div key={day} className="calendar-weekday">{day}</div>
                    ))}
                  </div>
                  
                  <div className="calendar-grid">
                    {generateCalendarGrid().map((dateInfo: CalendarDateInfo | null, index: number) => (
                      <button
                        key={index}
            ...
            <section className="active-sobrecupos">
              ...
              <div className="sobrecupos-list">
                {sobrecupos.length > 0 ? (
                  sobrecupos.slice(0, 2).map((sobrecupo: Sobrecupo, index: number) => (
            ...
            {fields.Seguros && fields.Seguros.length > 0 && (
              <section className="seguros-card">
                ...
                <div className="seguros-grid">
                  {(Array.isArray(fields.Seguros) ? fields.Seguros : [fields.Seguros]).map((seguro: string, index: number) => (
