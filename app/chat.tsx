...
export default function Chat({ initialMessage, onSessionUpdate }: ChatProps = {}): React.ReactElement {
  ...
  async function sendMessage(e?: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const myMsg: ChatMessage = { from: "user", text: input };
    ...
    if (
      ...
      try {
        ...
        const data: BotResponse = await aiRes.json();
        
        const botMessage: ChatMessage = {
          from: "bot",
          text: data.text || "¡Hola! 😊 ¿En qué te puedo ayudar? Cuéntame tus síntomas, el médico o especialidad que buscas y te ayudo a encontrar una hora disponible."
        };
    ...
    try {
      ...
      if (data.text) {
        ...
        const botMessages: ChatMessage[] = parts.map(text => ({ from: "bot", text }));
