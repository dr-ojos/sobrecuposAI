...
function ChatComponent() {
  ...
  // SIEMPRE generar nuevo sessionId en cada refresh - UX SIMPLE
  useEffect(() => {
    // SIEMPRE crear nueva sesión en cada refresh del navegador
    const newId = 'session_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    
    // Limpiar cualquier sesión anterior
    const oldId = localStorage.getItem('chatSessionId');
    if (oldId) {
      localStorage.removeItem(`chatSession_${oldId}`);
    }
    
    // Setear nueva sesión
    localStorage.setItem('chatSessionId', newId);
    setSessionId(newId);
    setSession({}); // Siempre empezar limpio
    
    console.log('🔄 Nueva conversación iniciada automáticamente:', newId);
