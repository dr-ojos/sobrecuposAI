...
// Interface for payment data from URL parameters
interface PaymentData {
  sobrecupoId: string | null;
...
// Interface for patient data in API calls
...
// Interface for appointment data in API calls
...
// Interface for payment simulation API response
...
// Interface for payment confirmation API response
interface PaymentConfirmationResponse {
  success: boolean;
  error?: string;
}

// Interface for appointment details in success message
...
// Interface for success message sent to chat
...
// Interface for error message sent to chat
...
// Interface for cancellation message sent to chat
interface PaymentCancelMessage {
  type: 'PAYMENT_CANCELLED';
  sessionId: string | null;
}

// Union type for all possible messages sent to chat
type ChatMessage = PaymentSuccessMessage | PaymentErrorMessage | PaymentCancelMessage;

function PagoContent(): React.JSX.Element {
  ...
  const [isFromChat, setIsFromChat] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    // Detect if coming from chat or directly
  ...
  const handlePaymentSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      const result: PaymentSimulationResponse = await response.json();
      ...
      if (result.success && result.transactionId) {
        ...
        // Confirm the reservation in the backend
        ...
        try {
          ...
          const confirmResult: PaymentConfirmationResponse = await confirmResponse.json();
          ...
          if (confirmResult.success) {
            ...
            if (isFromChat && window.opener) {
              ...
              // Give a moment for the message to be processed before closing
              setTimeout(() => {
                console.log('🔄 Cerrando ventana de pago...');
                // Try to focus parent window before closing
                if (window.opener && !window.opener.closed) {
                  window.opener.focus();
                }
                window.close();
              }, 1500); // Optimized for fast closure
              
            } else if (isFromChat) {
              // Chat flow without window.opener (fallback)
              console.log('❌ Chat sin window.opener - usando localStorage como fallback');
              
              const fallbackMessage: PaymentSuccessMessage = {
                type: 'PAYMENT_SUCCESS',
                transactionId: result.transactionId,
                sessionId: paymentData.sessionId,
                reservationConfirmed: true,
              ...
              // Direct reservation flow - redirect to confirmation page
            ...
            // DON'T CLOSE WINDOW when there's error to see logs
            
            // Notify the problem to chat if coming from chat
            if (isFromChat && window.opener) {
              const errorMessage: PaymentErrorMessage = {
                type: 'PAYMENT_SUCCESS_RESERVATION_ERROR',
                transactionId: result.transactionId,
                sessionId: paymentData.sessionId,
                error: confirmResult.error || 'Error desconocido'
              };
          ...
          addDebugLog(`❌ Error: ${(confirmError as Error).message}`);
          addDebugLog(`❌ Stack: ${(confirmError as Error).stack}`);
          setMessage('Pago exitoso pero error confirmando reserva. Contacta soporte.');
          console.error('❌ Catch error confirmando reserva:', confirmError);
          
          // DON'T CLOSE WINDOW when there's error to see logs
          
          // Notify error to chat if coming from chat
          if (isFromChat && window.opener) {
            const errorMessage: PaymentErrorMessage = {
              type: 'PAYMENT_SUCCESS_RESERVATION_ERROR',
              transactionId: result.transactionId,
        ...
        // Closure is handled individually in each case above
  ...
  const handleClose = (): void => {
    if (isFromChat) {
      // If coming from chat, notify and close window
      if (window.opener) {
        const cancelMessage: PaymentCancelMessage = {
          type: 'PAYMENT_CANCELLED',
      ...
      // If direct reservation, go back to main page
      router.push('/');
    }
  };
        ...
        {paymentStatus === 'pending' && (
          <>
            ...
            <div className="payment-form">
              ...
              {message && (
                <div className={`message ${(paymentStatus as PaymentStatus) === 'error' ? 'error' : ''}`}>
                  {message}
                </div>
              )}
...
const PagoPage = (): React.JSX.Element => {
  return (
    <Suspense fallback={
      <div style={{
        display: 'flex',
