...
interface SobrecupoFields {
  ...
  Dirección: string;
  Disponible: 'Si' | 'No';
  Seguro?: string | string[];
  Seguros?: string | string[];
  Previsiones?: string | string[];
  Isapres?: string | string[];
  Prevision?: string | string[];
  Atiende?: string;
}
...
const AgendarSobrecuposContent: React.FC = () => {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  // State with proper TypeScript types
  const [sobrecupos, setSobrecupos] = useState<Sobrecupo[]>([]);
  const [filteredSobrecupos, setFilteredSobrecupos] = useState<Sobrecupo[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [selectedSobrecupo, setSelectedSobrecupo] = useState<Sobrecupo | null>(null);
  ...
  const [calendarDates, setCalendarDates] = useState<Map<string, number>>(new Map());
    ...
    const fetchSobrecupos = async (): Promise<void> => {
      try {
        ...
        if (data.success) {
          ...
          const futureSobrecupos = records.filter((sobrecupo: Sobrecupo) => {
          ...
          futureSobrecupos.forEach((sobrecupo: Sobrecupo) => {
  ...
  useEffect(() => {
    ...
    const dateMap = new Map<string, number>();
    sobrecuposParaCalendario.forEach((sobrecupo: Sobrecupo) => {
      const date = sobrecupo.fields?.Fecha;
      if (date) {
        const count = dateMap.get(date) || 0;
        dateMap.set(date, count + 1);
  ...
  const handleReservarClick = (sobrecupo: Sobrecupo): void => {
    setSelectedSobrecupo(sobrecupo);
    setShowReservationModal(true);
  };

  const handleReservationSubmit = async (): Promise<void> => {
    if (!selectedSobrecupo) return;
    ...
    try {
      ...
      const paymentRequestData: PaymentLinkRequest = {
      ...
      const paymentResult: PaymentLinkResponse = await paymentResponse.json();
  ...
  const getUniqueEspecialidades = (): string[] => {
  ...
  const getSobrecuposCount = (dateString: string): number => {
  ...
  const handleCalendarDateClick = (dateString: string): void => {
  ...
  const generateCalendarGrid = (): (CalendarDateInfo | null)[] => {
    ...
    const grid: (CalendarDateInfo | null)[] = [];
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent): void => {
      ...
      if (showCalendar && !target.closest('.date-filter-container')) {
        setShowCalendar(false);
      }
    };
  ...
  const getPrevisiones = (sobrecupo: Sobrecupo): string => {
                ...
                <div className="form-row">
                  <div className="form-field">
                    ...
                    <input
                      ...
                      onChange={(e) => handleReservationDataChange('nombre', e.target.value)}
                  ...
                  <div className="form-field">
                    ...
                    <input
                      ...
                      onChange={(e) => handleReservationDataChange('apellidos', e.target.value)}
                      className="field-input"
                ...
                <div className="form-row">
                  <div className="form-field">
                    ...
                    <input
                      ...
                      onChange={(e) => handleReservationDataChange('rut', e.target.value)}
                      className="field-input"
                  ...
                  <div className="form-field">
                    ...
                    <input
                      ...
                      onChange={(e) => handleReservationDataChange('email', e.target.value)}
                      className="field-input"
                ...
                <div className="form-row">
                  <div className="form-field">
                    ...
                    <input
                      ...
                      onChange={(e) => handleReservationDataChange('telefono', e.target.value)}
                      className="field-input"
                  ...
                  <div className="form-field">
                    ...
                    <input
                      ...
                      onChange={(e) => handleReservationDataChange('edad', e.target.value)}
                      className="field-input"
                      placeholder="30"
                      min="1"
                      max="120"
...
const AgendarSobrecuposPage: React.FC = () => {
  return (
    <Suspense fallback={
      <div style={{
        display: 'flex',
