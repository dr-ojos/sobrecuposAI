...
function MedicoDashboard(): React.ReactElement {
  ...
  const fetchDoctorData = async (): Promise<void> => {
    ...
    if (!extendedSession?.user?.doctorId) {
      setLoading(false);
      return;
    }

    try {
      const res = await fetch(`/api/doctors/${extendedSession.user.doctorId}`);
      if (res.ok) {
        ...
        setStats(prev => ({
          ...prev,
          clinicas: (data.fields?.Clinicas as string[] | undefined)?.length || 0
        }));
      }
    } catch (error) {
  ...
  const fetchSobrecupos = async (): Promise<void> => {
    ...
    try {
      const sobrecuposUrl = `/api/sobrecupos/medicos/${extendedSession.user.doctorId}`;
