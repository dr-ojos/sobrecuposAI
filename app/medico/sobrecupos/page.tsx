// app/medico/sobrecupos/page.tsx
'use client';
import React from 'react';
import { useSession } from 'next-auth/react';
import { useRouter } from 'next/navigation';
import { useState, useEffect, FormEvent, ChangeEvent } from 'react';
import { SobrecupoItem, DoctorProfile } from '../../../types/dashboard';
import { NewSobrecupo, Clinica } from '../../../types/sobrecupos';
...
type FilterType = 'proximos' | 'disponibles' | 'reservados' | 'antiguos';
...
interface ExtendedUser {
  ...
  email?: string | null;
  doctorId?: string;
  doctorData?: DoctorProfile['fields'];
  userType?: string;
}
...
export default function SobrecuposMedico(): React.JSX.Element {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [loading, setLoading] = useState<boolean>(true);
  const [sobrecupos, setSobrecupos] = useState<SobrecupoItem[]>([]);
  const [filteredSobrecupos, setFilteredSobrecupos] = useState<SobrecupoItem[]>([]);
  const [clinicas, setClinicas] = useState<Clinica[]>([]);
  const [showCreateForm, setShowCreateForm] = useState<boolean>(false);
  const [message, setMessage] = useState<string>('');
  const [activeFilter, setActiveFilter] = useState<FilterType>('proximos');
  const [newSobrecupo, setNewSobrecupo] = useState<NewSobrecupo>({
    clinica: '',
    direccion: '',
    fecha: '',
    hora: '',
  ...
  const fetchSobrecupos = async (): Promise<void> => {
    try {
      ...
      const res = await fetch(`/api/sobrecupos/medicos/${typedSession.user.doctorId}`);
      if (res.ok) {
        const data: SobrecupoItem[] = await res.json();
        setSobrecupos(data);
      }
    } catch (error) {
  ...
  const filterSobrecupos = (): void => {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    
    let filtered: SobrecupoItem[] = [];
  ...
  const handleCreateSobrecupo = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    setMessage('');
    ...
    try {
      const typedSession = session as ExtendedSession;
      const doctorData = typedSession.user.doctorData;
      
      const res = await fetch('/api/sobrecupos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          medico: typedSession.user.doctorId,
  ...
  const handleClinicaSelect = (e: ChangeEvent<HTMLSelectElement>): void => {
  ...
  const getTimeFromNow = (dateStr: string, timeStr: string): string => {
    const now = new Date();
  ...
  const deleteSobrecupo = async (id: string): Promise<void> => {
  ...
  const getFilterCounts = (): FilterCounts => {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
      ...
      <main className="main-content">
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-field">
                      ...
                      <input
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setNewSobrecupo({...newSobrecupo, fecha: e.target.value})}
                        required
                        className="field-input"
                    ...
                    <div className="form-field">
                      ...
                      <select
                        ...
                        onChange={(e: ChangeEvent<HTMLSelectElement>) => setNewSobrecupo({...newSobrecupo, hora: e.target.value})}
                        required
                        className="field-input"
                      >
        ...
        <section className="filters-section">
          <div className="filters-container">
            {(['proximos', 'disponibles', 'reservados', 'antiguos'] as FilterType[]).map((filter) => (
              <button
                key={filter}
                      ...
                      <div className="patient-info">
                        ...
                        <div className="patient-details">
                          ...
                          {sobrecupo.fields?.Clínica && (
                            <div className="patient-contact">{sobrecupo.fields.Clínica}</div>
                          )}
