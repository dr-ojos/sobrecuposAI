...
import type { SobrecupoItem } from "../../../../../types/dashboard";
import type { CreateSobrecupoRequest, CreateSobrecupoResponse } from "../../../../../types/sobrecupos";

const AIRTABLE_API_KEY = process.env.AIRTABLE_API_KEY;
const AIRTABLE_BASE_ID = process.env.AIRTABLE_BASE_ID;
const AIRTABLE_TABLE_ID = process.env.AIRTABLE_TABLE_ID;

interface SobrecupoParams {
  params: {
    id: string;
  };
}
...
interface CreateSobrecupoSuccess {
  success: boolean;
  id: string;
  message: string;
  data: CreateSobrecupoResponse;
}
...
export async function GET(
  ...
  try {
    ...
    const sobrecupos: SobrecupoItem[] = data.records || [];
...
async function tryAlternativeFormula(doctorId: string): Promise<NextResponse<SobrecupoItem[] | ErrorResponse>> {
  try {
...
async function filterInJavaScript(doctorId: string): Promise<NextResponse<SobrecupoItem[]>> {
  try {
    ...
    const data = await res.json();
    const allSobrecupos: SobrecupoItem[] = data.records || [];
...
export async function POST(
  ...
  try {
    ...
    const body: CreateSobrecupoBody = await request.json();
    ...
    const record: CreateSobrecupoRequest = {
      fields: {
    ...
    const responseData: CreateSobrecupoResponse = await airtableResponse.json();
    ...
    if (!airtableResponse.ok) {
      ...
      return NextResponse.json(
        { 
          ...
          details: (responseData as any).error?.message || 'Error desconocido'
        }, 
        { status: 500 }
      );
    }
    ...
    console.error('❌ Error general creando sobrecupo:', err);
    return NextResponse.json(
      { 
        error: 'Error interno del servidor',
        message: err.message 
