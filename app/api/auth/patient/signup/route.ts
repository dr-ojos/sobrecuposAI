...
export async function POST(req: NextRequest): Promise<NextResponse<PatientSignUpResponse | ApiErrorResponse>> {
  try {
    const body: PatientSignUpRequest = await req.json();
    ...
    const patientData: PatientAirtableData = {
    ...
    const data: AirtableCreateResponse | AirtableError = await res.json();
